...
bool util::buffer_pool::create(size_t size, size_t buffer_size)
{
  // Initialize mutex.
...
util::buffer* util::buffer_pool::pop()
{
  pthread_mutex_lock(&_M_mutex);

  buffer* b = (_M_used > 0) ? _M_buffers[--_M_used] : nullptr;
...
util::buffer* util::buffer_pool::pop(unsigned timeout)
{
  pthread_mutex_lock(&_M_mutex);

  // If the buffer pool is not empty...
  if (_M_used > 0) {
    buffer* b = _M_buffers[--_M_used];
